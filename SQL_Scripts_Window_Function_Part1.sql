...
-- Select Hightest and lowest salary per deparment 
SELECT 
    e.*,
    MAX(salary) OVER(PARTITION BY dept_name) AS highest_salary,
    MIN(salary) OVER(PARTITION BY dept_name) AS lowest_salary
FROM 
    employee e;
...
select e.*,
  rank() over(partition by dept_name ) as rnk
from employee e;
